...
const char *Launcher::DataFoldersPage::mDefaultContentListName = "Default";

Launcher::DataFoldersPage::DataFoldersPage(Files::ConfigurationManager &cfg, Config::GameSettings &gameSettings, Config::LauncherSettings &launcherSettings, QWidget *parent)
    : QWidget(parent)
    , mCfgMgr(cfg)
...
{
    ui.setupUi (this);
    setObjectName ("DataFoldersPage");
    mSelector = new ContentSelectorView::ContentSelector (ui.folderSelectorWidget);
...
void Launcher::DataFoldersPage::buildView()
{
...
bool Launcher::DataFoldersPage::loadSettings()
{
...
void Launcher::DataFoldersPage::populateFileViews(const QString& contentModelName)
{
    ...
    mSelector->setProfileContent(filesInProfile(contentModelName));
}

QStringList Launcher::DataFoldersPage::filesInProfile(const QString& profileName)
{
    QStringList folders = mLauncherSettings.getDataFolders(profileName);
    return folders;
}

void Launcher::DataFoldersPage::saveSettings(const QString &profile)
{
   ...
   ContentSelectorModel::ContentFileList items = mSelector->selectedFiles();
    ...
    mLauncherSettings.setContentList(profileName, folderPaths);
    mGameSettings.setContentList(folderPaths);
}

void Launcher::DataFoldersPage::removeProfile(const QString &profile)
{
    mLauncherSettings.removeFilePathList(profile);
}

QAbstractItemModel *Launcher::DataFoldersPage::profilesModel() const
{
    return ui.profilesComboBox->model();
}

int Launcher::DataFoldersPage::profilesIndex() const
{
    return ui.profilesComboBox->currentIndex();
}

void Launcher::DataFoldersPage::setProfile(int index, bool savePrevious)
{
...
void Launcher::DataFoldersPage::setProfile (const QString &previous, const QString &current, bool savePrevious)
{
...
void Launcher::DataFoldersPage::slotProfileDeleted (const QString &item)
{
    removeProfile (item);
}

void Launcher::DataFoldersPage::slotProfileChangedByUser(const QString &previous, const QString &current)
{
    setProfile(previous, current, true);
    emit signalProfileChanged (ui.profilesComboBox->findText(current));
}

void Launcher::DataFoldersPage::slotProfileRenamed(const QString &previous, const QString &current)
{
    if (previous.isEmpty())
        return;
...
void Launcher::DataFoldersPage::slotProfileChanged(int index)
{
...
void Launcher::DataFoldersPage::on_newProfileAction_triggered()
{
...
void Launcher::DataFoldersPage::addProfile (const QString &profile, bool setAsCurrent)
{
    if (profile.isEmpty())
        return;
...
void Launcher::DataFoldersPage::on_deleteProfileAction_triggered()
{
...
void Launcher::DataFoldersPage::updateOkButton(const QString &text)
{
...
void Launcher::DataFoldersPage::checkForDefaultProfile()
{
...
bool Launcher::DataFoldersPage::showDeleteMessageBox (const QString &text)
{
